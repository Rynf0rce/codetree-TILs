    ...
    public static void main(String[] args) throws IOException{
        ...
        for(int k = 0 ; k < 2 ; k++){
            for(int i = 0 ; i < cpLength ; i++){
                hashCp[k] += (toInt(cpStr.charAt(i)) * pPow[k][cpLength - i - 1]) % MOD[k];
                hashIn[k] += (toInt(inStr.charAt(i)) * pPow[k][cpLength - i - 1]) % MOD[k];
            }
        }
        ...
        for(int i = 1 ; i <= inLength - cpLength ; i++){
            for(int k = 0 ; k < 2 ; k++){
                hashIn[k] = (hashIn[k] * P[k] - toInt(inStr.charAt(i - 1)) * pPow[k][cpLength] + toInt(inStr.charAt(i + cpLength - 1))) % MOD[k];
